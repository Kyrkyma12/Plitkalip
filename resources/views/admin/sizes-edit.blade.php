...
@section('content')
    <div class="main-content-inner">
        <div class="main-content-wrap">
            ...
            <!-- new-category -->
            <div class="wg-box">
                <form class="form-new-product form-style-1" action="{{ route('admin.size.update') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="id" value="{{ $size->id }}" />
                    <fieldset class="name">
                        <div class="body-title">Size Name <span class="tf-color-1">*</span></div>
                        <input class="flex-grow" type="text" placeholder="Size name" name="name" tabindex="0" value="{{ $size->name }}" aria-required="true" required="">
                    </fieldset>
                    @error('name')
                    ...
                    <fieldset class="name">
                        ...
                        <input class="flex-grow" type="text" placeholder="Size Slug" name="slug" tabindex="0" value="{{ $size->slug }}" aria-required="true" required="">
                    </fieldset>
                    @error('slug')
                    <span class="alert alert-danger text-center">{{$message}}</span>
                    @enderror
